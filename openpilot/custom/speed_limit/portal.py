...
@media(max-width:360px){.stats{gap:.4rem}.stat{padding:.5rem}.stat strong{font-size:1rem}.two{grid-template-columns:1fr}}
</style>
<body><main class=app>
...
<div class=field two><label class=input-label>Offset (mph)<input id=offset type=number inputmode=decimal step=.1></label><label class=input-label>Absolute maximum (mph)<input id=max type=number inputmode=decimal step=.1 placeholder="No maximum"></label></div>
</section>
...
function status(data){let settings=data.settings||{},rsa=data.rsa||{},device=data.device||{};
  ...
  $('offset').value=((settings.offset_mps||0)*MPS_TO_MPH).toFixed(1);$('max').value=settings.absolute_max_mps===null||settings.absolute_max_mps===undefined?'':(settings.absolute_max_mps*MPS_TO_MPH).toFixed(1);
...
async function save(){try{let maximum=$('max').value;let data=await api('/api/settings',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({
  enabled:$('enabled').checked,auto_accept_lower:$('lower').checked,auto_accept_higher:$('higher').checked,
  offset_mps:Number($('offset').value)/MPS_TO_MPH,absolute_max_mps:maximum===''?null:Number(maximum)/MPS_TO_MPH,
  ...
  local_phone_portal:$('portal-enabled').checked})});status(data);notice('Saved to your comma.','ok')}catch(error){notice(error.message,'error')}}
