...
<label><input id=enabled type=checkbox> Enable Toyota RSA controller</label><label>Offset (m/s)<input id=offset type=number step=.1></label><label>Absolute maximum (m/s; blank disables)<input id=max type=number step=.1></label><button onclick='save()'>Save while offroad</button>
<script>const $=id=>document.getElementById(id);async function api(path,options={}){options.headers={Authorization:'Bearer '+$('token').value,...(options.headers||{})};let r=await fetch(path,options);let j=await r.json();if(!r.ok)throw Error(j.error);return j}async function load(){try{let j=await api('/api/status');$('status').textContent=JSON.stringify(j,null,2);$('enabled').checked=j.settings.enabled;$('offset').value=j.settings.offset_mps;$('max').value=j.settings.absolute_max_mps??''}catch(e){$('status').textContent=e}}async function save(){try{let v=$('max').value;let j=await api('/api/settings',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled:$('enabled').checked,offset_mps:Number($('offset').value),absolute_max_mps:v===''?null:Number(v)})});$('status').textContent=JSON.stringify(j,null,2)}catch(e){$('status').textContent=e}}</script>"""
